...
Mesh::Mesh()
	: mVerts(0), mFaces(0), mTexture(nullptr), mDisplayListId(std::nullopt) {}
...
void Mesh::InitDisplayList() {
	...
	mDisplayListId = sDisplayListIds.front();
	sDisplayListIds.pop();
	...
	glDrawElements(GL_TRIANGLES, mFaces.size() * 3, GL_UNSIGNED_SHORT,
				   mFaces.data());
