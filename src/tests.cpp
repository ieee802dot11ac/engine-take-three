...
#define ENDTEST(name) ; std::cout << "\nBEGINNING TEST " #name "\n"; test_##name(); std::cout << "TEST " #name " SUCCEEDED\n";

#define FUNNY_ASSERT(expr) std::cout << #expr << "\n"; assert(bool(expr))
...
void do_tests() {
    ...
    TEST(vec_ops) {
        FUNNY_ASSERT(Vector2(3,4).Length() == 5);
        FUNNY_ASSERT(Vector2(1,0).Dot(Vector2(0,1)) == 1.f);
        FUNNY_ASSERT(Vector2(8, 234)[1] == 234);
        std::cout << std::endl;
