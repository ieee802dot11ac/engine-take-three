...
Object::~Object() {
	for (Object *obj : mChildObjs)
		delete obj;
	if (mParent != nullptr) {
		std::vector<Object *>::const_iterator end = mParent->mChildObjs.cend();
		for (std::vector<Object *>::iterator obj = mParent->mChildObjs.begin();
			 obj != end; obj++) {
			if (*obj == this)
				mParent->mChildObjs.erase(obj);
		}
	}
}
...
void Object::OnMsg(Message &msg) {
	for (Object *child : mChildObjs) { // how to decimate your cache 101
		child->OnMsg(msg);
	}
}
...
const Object *Object::FindByName(std::string name) const {
	if (mName == name) {
		...
		for (const Object *o : mChildObjs) {
			const Object *ptr;
			if ((ptr = o->FindByName(name)) != nullptr)
				return ptr;
			else
				continue;
		}
...
Object *Object::New(std::string cls_name) {
	...
	SDL_LogInfo(SDL_LOG_CATEGORY_SYSTEM,
				"probably fine, but class %s failed to new properly",
				cls_name.c_str());
	return nullptr;
}
